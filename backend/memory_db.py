...
class MemoryDB:
        ...
        with self.lock:
            ...
            self.cursor.execute('''
                CREATE TABLE IF NOT EXISTS user_profiles (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    face_id TEXT UNIQUE, -- ID from face_recognition or 'unknown'
                    name TEXT,
    ...
    def replace_facts(self, new_facts_list):
        """Replaces all facts with a new consolidated list."""
        with self.lock:
            try:
                self.cursor.execute("DELETE FROM facts")
                for fact in new_facts_list:
                    self.cursor.execute("INSERT INTO facts (content) VALUES (?)", (fact,))
                self.conn.commit()
                return True
            except Exception as e:
                print(f"[MEMORY DB ERROR] Failed to replace facts: {e}")
                self.conn.rollback()
                return False
    ...
    def get_user_profile(self, face_id):
        with self.lock:
            self.cursor.execute("SELECT id, name, attributes, last_seen, interaction_count FROM user_profiles WHERE face_id = ?", (face_id,))
    ...
    def update_user_profile(self, face_id, name=None, attributes=None):
        with self.lock:
            # Check if exists
            profile = self.get_user_profile(face_id)
            if profile:
                new_attrs = profile["attributes"]
                if attributes:
                    new_attrs.update(attributes)
                
                new_name = name if name else profile["name"]
                
                self.cursor.execute('''
                    UPDATE user_profiles 
                    SET name = ?, attributes = ?, last_seen = CURRENT_TIMESTAMP, interaction_count = interaction_count + 1
                    WHERE face_id = ?
                ''', (new_name, json.dumps(new_attrs), face_id))
            else:
                # Create new
                attrs = json.dumps(attributes if attributes else {})
                self.cursor.execute('''
                    INSERT INTO user_profiles (face_id, name, attributes, interaction_count)
                    VALUES (?, ?, ?, 1)
                ''', (face_id, name or "Unknown", attrs))
            
            self.conn.commit()

    def get_all_users(self):
        with self.lock:
            self.cursor.execute("SELECT face_id, name, last_seen FROM user_profiles ORDER BY last_seen DESC")
            return [{"face_id": r[0], "name": r[1], "last_seen": r[2]} for r in self.cursor.fetchall()]
